...
def cmdline(parser=None, io=False):

    if not parser:
        parser = cmdlineParser(io)
    ...
    if io and args.infile:
        infile = args.infile
    else:
        infile = config.filenames['schedule', 'input']

    config.source_date = time.ctime(os.path.getmtime(infile))

    (sessions, participants) = session.read(infile)

    return (args, sessions, participants)
